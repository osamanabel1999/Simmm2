...
class _ToPerformanceWidgetState extends State<ToPerformanceWidget> {
  // ============================================================
  // COLORS (الألوان الاحترافية لـ EFB)
  // ============================================================
  ...
  // ألوان الـ Snackbars الشيك
  static const Color cDanger = Color(0xFFE53935);
  static const Color cSuccess = Color(0xFF2563EB);
  ...
  // Segmented Control States (نفس القيم الحرفية القديمة للأكشن)
  String _config = 'CONF 1+F'; // ['CONF 1+F', 'CONF 2', 'CONF 3']
  String _antiIce = 'OFF'; // ['OFF', 'Engine', 'Total']
  String _airCond = 'ON'; // ['ON', 'OFF']
  String _rwyCond = 'DRY'; // ['DRY', 'WET']
  ...
  Future<void> _fetchLiveData() async {
    ...
    try {
      ...
      if (sbData['general'] == null || sbData['origin'] == null) {
        _showErrorSnackBar("No active flight plan found on SimBrief.");
        setState(() => _isFetchingData = false);
        return;
      }

      // Extract Origin Data (لأن ده إقلاع Departure)
      final origin = sbData['origin'];
      String originIcao = origin['icao_code']?.toString() ?? "";
      String rwyStr = origin['plan_rwy']?.toString() ?? "";
      ...
      if (tlr != null &&
          tlr['takeoff'] != null &&
          tlr['takeoff']['runway'] != null &&
          (tlr['takeoff']['runway'] as List).isNotEmpty) {
        var rwyData = tlr['takeoff']['runway'][0];
        // Convert feet to meters (TORA or Length)
        rwyLen = (double.tryParse(rwyData['length_tora']?.toString() ??
                    rwyData['length']?.toString() ??
                    "0") ??
                0) *
            0.3048;
      }
      ...
      // Update UI with SimBrief Data (حظر مساس Elevation و Slope بناءً على طلبك الصارم)
      ...
      if (originIcao.isNotEmpty) {
        final metarUrl = Uri.parse(
            "https://aviationweather.gov/api/data/metar?ids=$originIcao&format=json");
        final metarResponse =
        ...
        if (metarResponse.statusCode == 200) {
          ...
          if (metarList.isNotEmpty) {
            ...
            _showSuccessSnackBar(
                "Live data fetched for $originIcao successfully!");
          } else {
            _showErrorSnackBar("No METAR data found for $originIcao.");
          }
        }
      }
    } catch (e) {
  ...
  Widget build(BuildContext context) {
    return Container(
      ...
      child: SafeArea(
        // تم التغليف بـ Stack عشان نحط شريط الـ iOS تحت خالص
        child: Stack(
          children: [
            LayoutBuilder(
                    ...
                    children: [
                      ...
                      const Padding(
                        ...
                        child: Column(
                          ...
                          children: [
                            Text('T/O Performance Data Entry',
                                style: TextStyle(
                                    fontFamily: 'Inter',
                                    fontSize: 22,
                            ...
                            SizedBox(height: 4),
                            Text(
                                'Enter the parameters below or fetch live data to calculate takeoff performance.',
                                style: TextStyle(
                                    fontFamily: 'Inter',
                                    fontSize: 13,
                                    color: secondaryText)),
                      ...
                      SizedBox(
                        ...
                        child: ElevatedButton.icon(
                          ...
                          style: ElevatedButton.styleFrom(
                            backgroundColor:
                                blueBright, // لون عالي التباين ليلفت الانتباه
            ...
            // إضافة الشريط العائم (Home Indicator)
            Positioned(
              bottom: 0,
              left: 0,
              right: 0,
  ...
  Widget _buildAircraftPanel() {
    return Container(
      ...
      child: Column(
        crossAxisAlignment: CrossAxisAlignment.start,
        children: [
          _buildPanelHeader(Icons.flight_takeoff, 'Aircraft & Weather'),
          const SizedBox(height: 24),
          ...
          _buildTextFieldRow('Gross Weight', _gwCtrl,
              ...
              suffixLabel: '(Tons)'),
          const SizedBox(height: 16),
          _buildTextFieldRow('CG', _cgCtrl,
              isDecimal: true,
              isError: _hasAttemptedSubmit && _cgCtrl.text.isEmpty,
              suffixLabel: '(% MAC)'),
          const SizedBox(height: 20),
          _buildSegmentedRow('Configuration', ['CONF 1+F', 'CONF 2', 'CONF 3'],
              _config, (val) => setState(() => _config = val)),
          const SizedBox(height: 20),
          _buildSegmentedRow('A/ICE', ['OFF', 'Engine', 'Total'], _antiIce,
              (val) => setState(() => _antiIce = val)),
          const SizedBox(height: 20),
          _buildSegmentedRow('Air Cond', ['ON', 'OFF'], _airCond,
              (val) => setState(() => _airCond = val)),
          const Padding(
  ...
  Widget _buildRunwayPanel() {
    return Container(
      ...
      child: Column(
        crossAxisAlignment: CrossAxisAlignment.start,
        children: [
          _buildPanelHeader(Icons.add_road, 'Runway Conditions'),
          const SizedBox(height: 24),
          ...
          // تم الاحتفاظ بالقيم الأصلية (DRY, WET)
          _buildSegmentedRow('Runway Condition', ['DRY', 'WET'], _rwyCond,
              (val) => setState(() => _rwyCond = val)),
        ],
      ),
    );
  }
      ...
      child: ElevatedButton(
        onPressed: () async {
          if (_validateForm() && widget.onCalculatePressed != null) {
            ...
            double parsedCg = double.tryParse(_cgCtrl.text.trim()) ?? 0.0;
            double parsedRwyLen =
