...
class _NavaidTeleportWidgetState extends State<NavaidTeleportWidget> {
  ...
  double? _aircraftDistanceToNavaid(NavaidModel model) {
    ...
    if (aircraftLat == null ||
        ...
        aircraftLon < -180.0 ||
        aircraftLon > 180.0 ||
        (aircraftLat == 0.0 && aircraftLon == 0.0)) {
      return null;
    }
...
class Xplin extends StatefulWidget {
  final double? width;
  final double? height;
  final double? aircraftLatitude;
  final double? aircraftLongitude;
  ...
  final Future Function(double simulatorTimeSeconds)? onSimulatorTimeChanged;
  final String? simulatorTimeText;
  final Future Function(double groundSpeed)? onGroundSpeedChanged;
  ...
  const Xplin(
      ...
      this.onChartGateSelected,
      this.onSpeedSet,
      this.onSimulatorTimeChanged,
      this.simulatorTimeText,
      this.onGroundSpeedChanged,
  ...
  @override
  _XplinState createState() => _XplinState();
}

class _XplinState extends State<Xplin> {
  ...
  late double _simulatorTimeSeconds;
  late double _groundSpeed;
  VoidCallback? _appStateListener;

  @override
  void initState() {
    super.initState();
    _simulatorTimeSeconds = _readSimulatorTimeFromAppState();
    _groundSpeed = _readGroundSpeedFromAppState();

    _appStateListener = () {
      if (!mounted) return;
      final double newTime = _readSimulatorTimeFromAppState();
      final double newSpeed = _readGroundSpeedFromAppState();
      if (newTime != _simulatorTimeSeconds || newSpeed != _groundSpeed) {
        setState(() {
          _simulatorTimeSeconds = newTime;
  ...
  double _readSimulatorTimeFromAppState() {
    final dynamic value = FFAppState().efbSimulatorTimeSeconds;
    final double parsed = value is num ? value.toDouble() : 43200.0;
    return parsed.clamp(0.0, 86400.0).toDouble();
  }

  double _readGroundSpeedFromAppState() {
    final dynamic value = FFAppState().efbGroundSpeed;
    final double parsed = value is num ? value.toDouble() : 1.0;
    return parsed.clamp(1.0, 6.0).toDouble();
  }

  void _setSimulatorTime(double value) {
    final double seconds = (value / 60.0).round() * 60.0;
    final double clampedSeconds = seconds.clamp(0.0, 86400.0).toDouble();
    setState(() => _simulatorTimeSeconds = clampedSeconds);
    FFAppState().update(() {
      FFAppState().efbSimulatorTimeSeconds = clampedSeconds;
    });
    if (widget.onSimulatorTimeChanged != null) {
      widget.onSimulatorTimeChanged!(clampedSeconds);
    }
  }

  void _setGroundSpeed(double value) {
    final double speed =
        (((value * 10).round() / 10).clamp(1.0, 6.0)).toDouble();
    setState(() => _groundSpeed = speed);
    FFAppState().update(() {
      FFAppState().efbGroundSpeed = speed;
    });
    if (widget.onGroundSpeedChanged != null) {
      widget.onGroundSpeedChanged!(speed);
    }
  }
  ...
  Widget _buildSimulatorControls() {
    return Container(
      ...
      child: Row(
        children: [
          Expanded(child: _buildSimulatorTimeControl()),
          const SizedBox(width: 18),
          Expanded(child: _buildGroundSpeedControl()),
        ],
      ),
    );
  }

  Widget _buildSimulatorTimeControl() {
    return Column(
      mainAxisSize: MainAxisSize.min,
      crossAxisAlignment: CrossAxisAlignment.start,
      children: [
        Row(
          children: [
            const Icon(Icons.schedule, color: Color(0xFF7AA5D2), size: 14),
            const SizedBox(width: 6),
            const Text(
              'SIMULATOR TIME',
              style: TextStyle(
                color: Color(0xFF537396),
                fontSize: 9,
                fontWeight: FontWeight.w900,
                letterSpacing: 1.2,
              ),
            ...
            Container(
              ...
              decoration: BoxDecoration(
                ...
                border: Border.all(color: const Color(0xFF315174)),
                boxShadow: const [
                  BoxShadow(color: Color(0x334A90E2), blurRadius: 8),
                ],
              ),
              child: Text(
                widget.simulatorTimeText ?? '',
                style: const TextStyle(
        ...
        SliderTheme(
          data: SliderTheme.of(context).copyWith(
            ...
            trackHeight: 4,
            activeTrackColor: const Color(0xFF5A94E3),
            inactiveTrackColor: const Color(0xFF1A2C42),
            thumbColor: const Color(0xFF7AA5D2),
            overlayColor: const Color(0x337AA5D2),
            overlayShape: const RoundSliderOverlayShape(overlayRadius: 13),
            thumbShape: const RoundSliderThumbShape(enabledThumbRadius: 7),
            trackShape: const RoundedRectSliderTrackShape(),
          ),
          child: Slider(
            value: _simulatorTimeSeconds.toDouble(),
            min: 0,
            max: 86400,
            divisions: 1440,
            onChanged: (value) => _setSimulatorTime(value),
          ),
        ),
        Row(
          mainAxisAlignment: MainAxisAlignment.spaceBetween,
          children: const [
            Text('00:00',
                style: TextStyle(color: Color(0xFF324866), fontSize: 8)),
            Text('12:00',
                style: TextStyle(color: Color(0xFF537396), fontSize: 8)),
            Text('24:00',
                style: TextStyle(color: Color(0xFF324866), fontSize: 8)),
          ],
        ),
  ...
  Widget _buildGroundSpeedControl() {
    return Column(
      mainAxisSize: MainAxisSize.min,
      crossAxisAlignment: CrossAxisAlignment.start,
      children: [
        Row(
          children: [
            const Icon(Icons.speed, color: Color(0xFFE5B064), size: 14),
            const SizedBox(width: 6),
            const Text(
              'GROUND SPEED',
              style: TextStyle(
            ...
            Container(
              ...
              decoration: BoxDecoration(
                color: const Color(0xFF0D1A2A),
                borderRadius: BorderRadius.circular(5),
                border: Border.all(color: const Color(0xFF4A5870)),
                boxShadow: const [
                  BoxShadow(color: Color(0x33E5B064), blurRadius: 8),
                ],
              ),
              child: Text(
                '${_groundSpeed.toStringAsFixed(1)}x',
                style: const TextStyle(
                  color: Color(0xFFE5B064),
                  fontSize: 12,
                  fontWeight: FontWeight.w900,
  ...
  Widget _buildMobileSimulatorControls() {
    return Container(
      width: double.infinity,
      ...
      decoration: BoxDecoration(
        ...
        border: Border.all(color: const Color(0xFF1E324A), width: 1.0),
        boxShadow: const [
          BoxShadow(color: Color(0x225A94E3), blurRadius: 12, spreadRadius: 1),
        ],
      ),
      child: Row(
        children: [
          Expanded(child: _buildMobileSimulatorTimeControl()),
          const SizedBox(width: 7),
          Expanded(child: _buildMobileGroundSpeedControl()),
        ],
      ),
    );
  }

  Widget _buildMobileSimulatorTimeControl() {
    return Column(
      mainAxisSize: MainAxisSize.min,
      crossAxisAlignment: CrossAxisAlignment.start,
      children: [
        Row(
          children: [
            const Icon(Icons.schedule, color: Color(0xFF7AA5D2), size: 12),
            ...
            Container(
              ...
              child: Text(widget.simulatorTimeText ?? '',
                  style: const TextStyle(
                      color: Color(0xFFA6C2DF),
                      fontSize: 8,
                      fontWeight: FontWeight.w900,
        ...
        Row(
          ...
          children: const [
            Text('00:00',
                style: TextStyle(color: Color(0xFF324866), fontSize: 7)),
            Text('12:00',
                style: TextStyle(color: Color(0xFF537396), fontSize: 7)),
            Text('24:00',
                style: TextStyle(color: Color(0xFF324866), fontSize: 7)),
          ],
        ),
  ...
  Widget _buildMobileGroundSpeedControl() {
    return Column(
      mainAxisSize: MainAxisSize.min,
      crossAxisAlignment: CrossAxisAlignment.start,
      children: [
        Row(
          children: [
            const Icon(Icons.speed, color: Color(0xFFE5B064), size: 12),
            const SizedBox(width: 4),
            const Expanded(
              child: Text('GROUND SPD',
                  maxLines: 1,
                  overflow: TextOverflow.ellipsis,
                  style: TextStyle(
                      color: Color(0xFF537396),
            ...
            Container(
              ...
              child: Text('${_groundSpeed.toStringAsFixed(1)}x',
                  style: const TextStyle(
                      color: Color(0xFFE5B064),
                      fontSize: 9,
                      fontWeight: FontWeight.w900,
              ...
              child: ClipRRect(
                ...
                child: Stack(
                  children: [
                    ...
                    Offstage(
                        offstage: _selectedMode != 1, child: _buildGatesArea()),
                    Offstage(
